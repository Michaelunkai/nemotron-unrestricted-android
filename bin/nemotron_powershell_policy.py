...
def _validate_read_only_segment(segment: str) -> None:
    ...
    if name == "netsh":
        ...
        if (
            normalized[:4] == ["advfirewall", "firewall", "show", "rule"]
            and all(
                argument == "verbose" or argument.startswith("name=")
                for argument in normalized[4:]
            )
            and any(argument.startswith("name=") for argument in normalized[4:])
        ):
            return
        _block("netsh_not_read_only")
...
def validate_powershell_command(command: object) -> str:
    ...
    if lowered.startswith("netsh "):
        _validate_read_only_segment(command)
        return command
...
_NEMOTRON_WINDOWS_TEMP = re.compile(
    r"^nemotron-v[0-9.-]+-(?:clean|assets)-[0-9a-f]{8}$",
    re.IGNORECASE,
)
...
def _reject_protected_cleanup_path(path: str) -> list[str]:
    ...
    exact_nemotron_windows_temp = bool(
        len(parts) == 3
        and parts[:2] == ["windows", "temp"]
        and _NEMOTRON_WINDOWS_TEMP.fullmatch(parts[2])
    )
    if not parts or (parts[0] in _PROTECTED_TOP_LEVEL and not exact_nemotron_windows_temp):
...
function Test-ProtectedLiteralPath([string]$LiteralPath) {
    ...
    if ($Path -match '^[a-z]:\\windows\\temp\\nemotron-v[0-9.-]+-(?:clean|assets)-[0-9a-f]{8}$') { return $false }
