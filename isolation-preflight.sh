...
WEB_ASSET_VERSION=$(sha256sum "$WEB_SOURCE" | awk '{print substr($1, 1, 16)}')
WEB_SCRIPT_TAG="<script src=\"/nemotron-autonomy-progress.js?v=$WEB_ASSET_VERSION\"></script>"
...
grep -Fq "$WEB_SCRIPT_TAG" "$WEB_INDEX" || { printf 'ERROR: Nemotron web overlay is missing or cache-stale\n' >&2; exit 1; }
