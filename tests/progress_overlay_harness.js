...
function createEnvironment(options) {
  ...
  const windowObject = {
    ...
    NemotronAutonomy: {
      missionStarted() {},
      missionProgress(token, payload) {
        assert.strictEqual(token, "fixture-token");
        progressPayloads.push(JSON.parse(payload));
      },
...
async function main() {
  ...
  const switchInterrupts = env.fetchCalls.filter((entry) =>
    entry.rpc && entry.rpc.method === "turn/interrupt");
  assert.strictEqual(
    env.fetchCalls.filter((entry) => entry.rpc).length,
    rpcCountBeforeSwitch + 1,
    "an active selector change must issue exactly one additional RPC",
  );
  assert.strictEqual(switchInterrupts.length, 1);
  assert.deepStrictEqual(switchInterrupts[0].rpc.params, {threadId: "thread-a", turnId: "turn-a"});
  assert.strictEqual(
    env.fetchCalls.filter((entry) => entry.rpc && entry.rpc.method === "turn/start").length,
    1,
    "selector changes must not replay or start another turn",
  );
  assert.strictEqual(
  ...
  assert(snapshot.meaningfulUpdates.some((entry) => entry.label.includes("Stopping the active provider call now")));
  assert(snapshot.meaningfulUpdates.some((entry) => entry.label.includes("Stop request verified")));
  ...
  const commandProgress = reloaded.progressPayloads.at(-1);
  assert.strictEqual(commandProgress.state, "working");
  assert.strictEqual(commandProgress.technicalCategory, "command");
  assert.match(commandProgress.message, /Reading|Checking|Verifying/);
  assert.ok(!JSON.stringify(commandProgress).includes(liveCommand.command), "raw command escaped into progress");
  assert.ok(!Object.hasOwn(commandProgress, "rawCommand"));
  assert.ok(!Object.hasOwn(commandProgress, "rawOutput"));
