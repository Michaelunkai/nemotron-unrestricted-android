...
class ProgressOverlayTests(unittest.TestCase):
    ...
    def test_versioned_epoch_persistence_is_hydrated_before_transport_hooks(self):
        self.assertIn("const VERSION = '5.0.0'", self.source)
        self.assertIn("window.__NEMOTRON_AUTONOMY_PROGRESS__ === 'ready'", self.source)
        self.assertIn("window.__NEMOTRON_AUTONOMY_PROGRESS__ = 'loading'", self.source)
        self.assertIn("window.__NEMOTRON_AUTONOMY_PROGRESS__ = 'ready'", self.source)
    ...
    def test_truthful_internal_progress_and_recovery_are_preserved_without_floating_ui(self):
        self.assertIn("latestVerifiedResult", self.source)
        self.assertIn("nextBoundedAction", self.source)
        self.assertIn("meaningfulUpdates", self.source)
        self.assertIn("technicalEvents", self.source)
        self.assertIn("progressSequence", self.source)
        self.assertIn("missionProgress", self.source)
        self.assertIn("technicalCategory", self.source)
        self.assertNotIn("rawCommand:", self.source)
        self.assertNotIn("rawOutput:", self.source)
        self.assertIn("function removeFloatingUi()", self.source)
    ...
    def test_terminal_success_requires_substantive_agent_output(self):
        ...
        self.assertIn("outcome === 'stopped' ? 'stopped' : 'failed'", self.source)
    ...
    def test_active_runtime_change_stops_the_exact_turn_before_continuing(self):
        switch = self.source[
            self.source.index("async function stopActiveTurnForRuntimeSwitch"):
            self.source.index("async function runtimeIdentity")
        ]
        self.assertIn("validThreadId(state.currentThreadId)", switch)
        self.assertIn("validThreadId(state.turnId)", switch)
        self.assertIn("turnId.startsWith('pending-')", switch)
        self.assertIn("runtimeSwitchInterrupts.has(turnId)", switch)
        self.assertIn("await rpc('turn/interrupt', {threadId, turnId})", switch)
        self.assertIn("next explicit continuation", switch)
        observer = self.source[
            self.source.index("function installExactSelectionObserver"):
            self.source.index("const initialThreadId")
        ]
        self.assertIn("const activeAtSelection = state.active", observer)
        self.assertIn("stopActiveTurnForRuntimeSwitch(model, effort)", observer)
        self.assertNotIn("same turn continues", observer)
        self.assertNotIn("applies to the next provider continuation in this running turn", observer)

    def test_read_only_browser_ready_state_constants_cannot_abort_bootstrap(self):
        self.assertNotIn("NemotronObservedWebSocket[key] = NativeWebSocket[key]", self.source)
        self.assertNotIn("NemotronObservedEventSource[key] = NativeEventSource[key]", self.source)
        self.assertIn("Object.defineProperty(NemotronObservedWebSocket, key", self.source)
        self.assertIn("Object.defineProperty(NemotronObservedEventSource, key", self.source)
    ...
    def test_session_cleanup_is_scoped_backed_up_and_verified(self):
        ...
        self.assertNotIn("requestCleanupConfirmation", self.source)
        self.assertNotIn("DELETE ALL SESSIONS AND THREADS", self.source)
        self.assertNotIn("nemotron-cleanup-confirmation", self.source)
        self.assertIn("thread/delete", self.source)
        self.assertIn("nemotron-autonomy:sessions-deleted", self.source)
        self.assertIn("deletedThreadIds", self.source)
        self.assertIn("clearAllThreads", self.source)
        self.assertIn("pendingApprovalThreadIds", self.source)
        self.assertIn("approval-bearing sessions are preserved", self.source)
        self.assertIn("threadUiStorageKeys.forEach((key) => storage.removeItem(key))", self.source)
        self.assertIn("if (deletedIds.includes(selected)) storage.removeItem(selectedKey)", self.source)
        self.assertNotIn("storage.removeItem('codex-web-local.project-order.v1')", self.source)
        self.assertIn("retained-before-delete", self.source)
        cleanup = self.source[
    ...
    def test_chat_screen_has_no_floating_autonomy_controls_or_receipt(self):
        self.assertNotIn("state.panel = document.createElement", self.source)
        self.assertNotIn("state.toolbar = document.createElement", self.source)
        self.assertNotIn("#nemotron-autonomy-tools { position:fixed", self.source)
        self.assertNotIn("#${PANEL_ID}.na-floating", self.source)
        self.assertIn("function removeFloatingUi()", self.source)
    ...
    def test_deployed_asset_matches_source_without_pre_mount_html_execution(self):
