...
class GalleryTests(unittest.TestCase):
    ...
    def test_face_detector_is_batched_and_reports_exact_progress(self):
        paths = [f"/storage/emulated/0/DCIM/Camera/{index}.jpg" for index in range(11)]
        first = types.SimpleNamespace(
            remote_status=0,
            stdout="\n".join(json.dumps({"path": path, "faceCount": 1}) for path in paths[:8]),
        )
        second = types.SimpleNamespace(
            remote_status=0,
            stdout="\n".join(json.dumps({"path": path, "faceCount": 0}) for path in paths[8:]),
        )
        output = io.StringIO()
        ...
        self.assertIn("images 1–8 of 11", output.getvalue())
        self.assertIn("images 9–11 of 11", output.getvalue())

    def test_face_detector_retries_omitted_receipts_individually(self):
        paths = ["/storage/emulated/0/DCIM/Camera/a.jpg", "/storage/emulated/0/DCIM/Camera/b.jpg"]
        partial = types.SimpleNamespace(
            remote_status=0, stdout=json.dumps({"path": paths[0], "faceCount": 0}),
        )
        retry = types.SimpleNamespace(
            remote_status=0, stdout=json.dumps({"path": paths[1], "faceCount": 1}),
        )
        with mock.patch.object(GALLERY, "run_rish", side_effect=[partial, retry]) as run:
            result = GALLERY.detect_face_batches(paths, "/data/local/tmp/detector.jar")
        self.assertEqual(len(run.call_args_list), 2)
    ...
    def test_face_verification_counts_unique_paths_not_duplicate_media_rows(self):
        ...
        receipts = {paths[0]: {"path": paths[0], "faceCount": 0}}
