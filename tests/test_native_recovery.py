...
class NativeRecoveryTests(unittest.TestCase):
    ...
    def test_javascript_bridge_posts_sanitized_ordered_progress(self):
        bridge = self.source[
            self.source.index("public final class NemotronAutonomyBridge"):
            self.source.index("@android.annotation.TargetApi(26)")
        ]
        self.assertIn("public void missionProgress(String token, String payload)", bridge)
        self.assertIn("sanitizeProgressPayload", bridge)
        self.assertIn('postSupervisorEvent("progress", safePayload)', bridge)
