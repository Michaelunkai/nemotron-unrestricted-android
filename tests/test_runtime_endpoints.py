...
class RuntimeEndpointTests(unittest.TestCase):
    ...
    def test_runtime_selection_returns_structured_unsupported_effort_error(self):
        port = self.run_server(PROXY.Server(("127.0.0.1", 0), PROXY.Handler))
        with mock.patch.object(
            PROXY,
            "update_runtime_selection",
            side_effect=PROXY.InvalidReasoningEffortError("Max effort is unsupported for this model"),
        ):
            response, body = self.request_json(
                port,
                {"model": "provider/model", "effort": "max"},
                handler_path="/v1/runtime-selection",
            )
        payload = json.loads(body)
        self.assertEqual(response.status, 422)
        self.assertEqual(payload["error"]["type"], "unsupported_effort_error")
        self.assertIn("Max effort", payload["error"]["message"])
    ...
    def test_models_endpoint_refreshes_public_catalog_and_returns_dynamic_models(self):
        rows = [
            ...
            {"id": PROXY.EXACT_NEMOTRON_MODEL, "supported_parameters": ["tools", "reasoning", "max_tokens"], "reasoning": {"supports_max_tokens": True}, "pricing": {"prompt": "0.0000006", "completion": "0.0000036"}},
        ...
        with mock.patch.object(PROXY, "exact_nemotron_capability", return_value=verified_capability):
            first_response, first_payload = self.local_get_json(port, "/v1/models")
            self.assertEqual(first_response.status, 200)
            self.assertEqual(first_payload["catalog_source"], "live")
            self.assertIn(PROXY.EXACT_NEMOTRON_MODEL, [model["id"] for model in first_payload["data"]])
        ...
        self.assertEqual([model["id"] for model in payload["data"]], [
            PROXY.DEFAULT_MODEL,
            PROXY.EXACT_NEMOTRON_MODEL,
            "nvidia/nemotron-nano:free",
        ])
        self.assertEqual(payload["exact_nemotron_status"], {
            "available": True,
    ...
    def test_models_endpoint_hides_exact_model_when_credential_limit_is_exhausted(self):
        rows = [{
        ...
        with mock.patch.object(PROXY, "exact_nemotron_capability", return_value=exhausted_capability):
            response, payload = self.local_get_json(port, "/v1/models")
        self.assertEqual(response.status, 200)
        self.assertEqual(calls, [("catalog", None)])
        self.assertNotIn(PROXY.EXACT_NEMOTRON_MODEL, [model["id"] for model in payload["data"]])
        self.assertEqual(payload["exact_nemotron_status"], {
            "available": False,
            "provider": None,
            "providerVerified": False,
