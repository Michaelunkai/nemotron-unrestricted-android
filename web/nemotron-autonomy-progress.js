(function nemotronAutonomyProgressBootstrap() {
  ...
  function persistProgress() {
    ...
    const payload = {
      ...
      progressSequence: Math.max(0, Number(state.progressSequence) || 0),
  ...
  function hydratePersistedProgress(threadId) {
    ...
    state.progressSequence = Math.max(0, Number(saved.progressSequence) || 0);
    ...
    if (state.active && state.turnId) notifyMissionStarted(state.turnId);
    return true;
  }
  ...
  function progressCategory(identity, label) {
    const value = `${safeText(identity, 360)} ${safeText(label, 320)}`.toLowerCase();
    if (/command|shell|exec|build|install|download|search|fetch|android|windows|repository/u.test(value)) return 'command';
    if (/plan|step/u.test(value)) return 'plan';
    if (/retry|recover|reconcil|failure|interrupt/u.test(value)) return 'recovery';
    if (/verify|evidence|receipt|health|identity|complete|terminal/u.test(value)) return 'verification';
    if (/runtime|provider|model|effort/u.test(value)) return 'runtime';
    if (/reason/u.test(value)) return 'reasoning';
    return 'tool';
  }

  function publishMeaningfulProgress(entry) {
    const turnId = safeText(state.turnId, 256);
    const threadId = validThreadId(state.currentThreadId);
    if ((!state.active && !state.completed) || !turnId || !threadId || turnId.startsWith('pending-')
        || !registeredActiveTurns.has(turnId) || !entry) return;
    if (!window.NemotronAutonomy || typeof window.NemotronAutonomy.missionProgress !== 'function') return;
    state.progressSequence += 1;
    const status = /^(?:completed|failed|retry)$/u.test(entry.status) ? entry.status : 'working';
    const message = safeText(entry.label, 320) || 'Continuing the requested task';
    const eventId = `progress-${fingerprintDigest(`${turnId}:${state.progressSequence}:${entry.id}:${status}:${message}`)}-${state.progressSequence}`;
    try {
      window.NemotronAutonomy.missionProgress(window.__NEMOTRON_BRIDGE_TOKEN__ || '', JSON.stringify({
        schemaVersion: 1,
        eventId,
        sequence: state.progressSequence,
        threadId,
        turnId,
        actionId: safeText(entry.id, 256),
        state: status,
        message,
        verifiedResult: status === 'completed' ? safeText(entry.evidence || state.latestVerifiedResult, 320) : '',
        nextAction: status === 'working' || status === 'retry' ? safeText(state.nextBoundedAction, 320) : '',
        technicalCategory: progressCategory(entry.id, message),
      }));
    } catch (_) {
    }
  }
  ...
  function recordMeaningfulUpdate(label, status = 'active', identity = '', evidence = '') {
    ...
    state.meaningfulUpdates.push(entry);
    ...
    publishMeaningfulProgress(entry);
  }
  ...
  function notifyMissionStarted(turnId) {
    ...
    publishMeaningfulProgress({
      id: 'turn-started',
      label: 'Starting the requested task and checking the first verified action',
      status: 'active',
      evidence: '',
    });
  }
  ...
  function handleEvent(event, source = 'live') {
    ...
    if (method === 'item/started') {
      ...
      if (type === 'contextcompaction') {
        ...
        addStep(humanizeCommand(item.command), `command:${fingerprintDigest(key || itemId)}`);
  ...
  const runtimeSwitchInterrupts = new Set();

  async function stopActiveTurnForRuntimeSwitch(model, effort) {
    const threadId = validThreadId(state.currentThreadId);
    const turnId = validThreadId(state.turnId);
    if (!state.active || !threadId || !turnId || turnId.startsWith('pending-')) return false;
    if (runtimeSwitchInterrupts.has(turnId)) {
      setToolbarStatus(`Runtime updated: ${model} · ${effortLabel(effort)} · active provider stop already requested`);
      return true;
    }
    runtimeSwitchInterrupts.add(turnId);
    state.interruptionRequested = true;
    recordMeaningfulUpdate(
        `Stopping the active provider call now so ${model} · ${effortLabel(effort)} becomes effective without mixing runtimes`,
        'active', `runtime-switch-stop:${turnId}`);
    setToolbarStatus(`Stopping active provider call · switching to ${model} · ${effortLabel(effort)}`);
    persistProgress();
    render();
    try {
      await rpc('turn/interrupt', {threadId, turnId});
      recordMeaningfulUpdate(
          `Stop request verified for the previous provider call; ${model} · ${effortLabel(effort)} is selected for the next explicit continuation`,
          'completed', `runtime-switch-stopped:${turnId}`);
      setToolbarStatus(`Previous provider call stopped · selected ${model} · ${effortLabel(effort)}`);
      persistProgress();
      render();
      return true;
    } catch (error) {
      runtimeSwitchInterrupts.delete(turnId);
      state.interruptionRequested = false;
      throw new Error(`Runtime changed, but the active provider call could not be stopped safely: ${clean(error && error.message)}`);
    }
  }
  ...
  function installExactSelectionObserver() {
    ...
    window.addEventListener('nemotron:runtime-selection', (event) => {
      ...
      void queueRuntimeSelection({model, effort}).then(() => {
          ...
          state.queuedEffort = '';
          const activeAtSelection = state.active;
          recordMeaningfulUpdate(
              activeAtSelection
                ? `Runtime selected: ${model} · ${effortLabel(effort)}; stopping the active provider call before it can continue`
                : `Runtime selected: ${model} · ${effortLabel(effort)}`,
              activeAtSelection ? 'active' : 'completed', `runtime-live:${model}:${effort}`);
          setToolbarStatus(activeAtSelection
            ? `Runtime selected · stopping active call for ${model} · ${effortLabel(effort)}`
            : `Runtime selected: ${model} · ${effortLabel(effort)}`);
          persistProgress();
          render();
          if (activeAtSelection) return stopActiveTurnForRuntimeSwitch(model, effort);
          return false;
        }).catch((error) => {
