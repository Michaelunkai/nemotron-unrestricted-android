...
- `codex-scaffold list|create|verify|rollback` provides reviewed Python, Node, shell, static web, Go, Rust, Java, and PowerShell templates. Creation validates a workspace-confined new target, tests in staging, initializes Git, hashes the generated tree, and records rollback metadata; rollback refuses any project changed after creation.
- `codex-toolchain inspect|verify` detects and verifies Python, Node.js, shell, Java, Kotlin, Android, PowerShell, and static-web projects. Adapters use fixed argv or bounded parsers, avoid dependency installation and build-tree mutation, hash verified inputs, expose native-runtime readiness separately, and fail explicitly when an optional compiler is absent.
...
- `codex-intent open-url|open-app|settings|share-text|view|capture-image` builds typed Android activity specs for common workflows. Default mode resolves without opening UI; execution is explicit. It forbids file/credentialed URIs, supports typed content-URI output, redacts payload values from receipts, and delegates to `codex-android` postcondition verification.
- `codex-ui-safe snapshot|run` provides the Shizuku/uiautomator fallback when the richer accessibility broker is absent. Workflows are foreground-package scoped, exact-selector first, retry bounded, postcondition mandatory, and allow coordinates only as an explicit fallback. Snapshots/evidence are hashed and typed text values are omitted from receipts.
- `codex-device refresh|status|search` stores TTL-bound Android identity, user-0 packages, hardware/software features, Termux API commands, and readiness. The raw build fingerprint is hashed; identity changes invalidate dependent records; Shizuku, storage, battery, and package readiness come from verified read-only receipts.
...
- `codex-gallery metadata|ocr|export|share` reads exact EXIF/container metadata, transcribes visible text through the current verified vision route, creates checksum-verified non-destructive originals or JPEG/PNG/WebP copies, and dispatches Android shares while reporting any remaining user target-selection gate.
...
- `codex-gallery faces --offset N --limit N` performs batched local-only Android face-presence counting. `codex-gallery semantic '<visible details>' --offset N --limit N` uses the current verified zero-price vision route for visible object/scene/text matching. Both print exact English progress, return total/offset/`hasMore`/`nextOffset`, and create checksum-verified contact-sheet paths under `workspace/gallery-presentations/`; the agent must render every returned sheet inline and continue all pages. Neither route claims identity or sensitive-attribute inference.
...
- Mid-turn changes are scoped to one registered active thread and apply at its next provider continuation without cancelling or replaying the logical turn.

## Persistent jobs, recovery, goals, and learning

- Project-local `codex-job`, `codex-goal`, `codex-recover`, `codex-capability`, `codex-artifact`, `codex-verify`, `codex-browser`, `codex-ui`, `codex-network`, `codex-account`, and `codex-entitlement` use an allowlisted, resolved-target-checked delegate and Nemotron's isolated `CODEX_RUNTIME_ROOT`; scheduling, audit, undo/restore, health, verification, and recovery remain durable without leaking state into unrelated sessions.
