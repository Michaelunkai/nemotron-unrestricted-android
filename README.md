...
The current release is v1.5.0/code 6 at `dist/Nemotron-Unrestricted-1.5.0.apk`, SHA-256 `0f320c6385836f869d25390648853cad666395058f558f3f961c0e4172b82235`. It preserves the completed white-GUI recovery, one-click session/thread cleanup, zero-floating-control UI, model/effort switching, background supervisor, and terminal completion audio while adding resumable gallery presentation jobs, project-isolated durable capability routes, and a redesigned adaptive icon.

Release acceptance passed 347 automated tests, source validation, off-device 945×2048 UI goldens, the progress harness, source and signed-APK secret scans, package/version/signature checks, and deterministic icon verification. The signed APK is then installed in place, read back by exact hash/signer/version, and verified on a private virtual display without foregrounding the live app.
...
The public source repository is [Michaelunkai/nemotron-unrestricted-android](https://github.com/Michaelunkai/nemotron-unrestricted-android). The universal Android 1.5.0 APK is available from the [v1.5.0 release](https://github.com/Michaelunkai/nemotron-unrestricted-android/releases/tag/v1.5.0):

- [Download Nemotron-Unrestricted-1.5.0.apk](https://github.com/Michaelunkai/nemotron-unrestricted-android/releases/download/v1.5.0/Nemotron-Unrestricted-1.5.0.apk)
...
- Resumable MediaStore face-presence and non-sensitive visual searches with bounded batches, action-specific English progress, continuation offsets, checksum-verified contact sheets, and mandatory inline rendering of every result page.
- Project-local, target-verified delegates for durable jobs, goals, recovery, capability/artifact verification, browser/UI/network evidence, and account/entitlement status, all pinned to Nemotron's isolated runtime root.
...
codex-install dist/Nemotron-Unrestricted-1.5.0.apk \
  --package com.michaelovsky.nemotronunrestricted.isolated \
  --signer "$EXPECTED_SIGNER_SHA256" \
  --sha256 "0f320c6385836f869d25390648853cad666395058f558f3f961c0e4172b82235"
```
